...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $query = "SELECT * FROM salon_owner WHERE owner_name = :owner_name";
        ...
        if ($statement->rowCount() > 0) {
            ...
            if (password_verify($password, $admin['password'])) {
                // A jelszó helyes és már hashelt
                $_SESSION['owner_name'] = $owner_name;
                $_SESSION['salon_owner_id'] = $admin['salon_owner_id'];
                header("Location: edit_salon1.php");
                exit();
            } elseif ($admin['password'] === $password) {
                // A jelszó még nincs hashelve, most hasheljük és frissítjük az adatbázist
                $hashed_password = password_hash($password, PASSWORD_DEFAULT);

                $updateQuery = "UPDATE salon_owner SET password = :password WHERE salon_owner_id = :salon_owner_id";
                $updateStatement = $pdo->prepare($updateQuery);
                $updateStatement->bindParam(':password', $hashed_password);
                $updateStatement->bindParam(':salon_owner_id', $admin['salon_owner_id']);
                $updateStatement->execute();

                // Beléptetjük a felhasználót a jelszó hashelése után
                $_SESSION['owner_name'] = $owner_name;
                $_SESSION['salon_owner_id'] = $admin['salon_owner_id'];
                header("Location: edit_salon1.php");
                exit();
        ...
        <div class="pt-5 pb-5 text-center">
            <div class="container d-flex justify-content-center pt-5 pb-5">
                <div class="form-container mx-auto form-heading py-5">
                    ...
                    <div class="card-body">
                        <form action="salon_login.php" method="POST" class="row g-3">
                            <div class="mb-3">
