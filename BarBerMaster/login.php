...
date_default_timezone_set('Europe/Belgrade');
 function collectdata($dsn,$pdoOptions,$device_repo,$username)
    {
        try {
            ...
            $query = "INSERT INTO kv.user_info (ip, device, os, browser, date, username) VALUES (:ip, :device, :os, :browser, :date, :username)";
            ...
            $stmt->bindParam(':username', $username); //ezt adtam hozza plusszba
        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                if (password_verify($password, $result['password'])) {
                    ...
                    collectdata($dsn,$pdoOptions,$device_repo,$username); //a szerverre ez a rész nincs feltöltve(de nem is merem) nah majd most.
                    exit();
                } else {
                    $errors[] = 'Hibás jelszó';
                }
...
<body>
    ...
    <?php
// "I dont know why, I dont want to know why, I shouldn't have wonder why" de ez minek van itt?
    ...
    if (isset($_POST['send-link'])) {
        ...
        try {
            ...
            if (!$existingUser) {
                echo "
                <script>
                    alert('Nincs ilyen email!');
                </script>";
                exit;
            } elseif ($existingUser["active"] == 0) {
                echo "
                <script>
                    alert('Először kérlek verifikáld magad!');
                </script>";
                exit;
            } else {
            ...
            <p>Az alábbi linken tudod visszaállítani a jelszavad!</p>
            https://kv.stud.vts.su.ac.rs/BarBerMaster/updatepw.php?token=" . $pw_token;
