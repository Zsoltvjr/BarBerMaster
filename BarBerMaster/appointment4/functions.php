...
function getReservationsData($date)
{
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT start_time AS reservation_time FROM reservations WHERE date = ? AND cancelled = 0 AND store_id = 4");
        $stmt->execute([$date]);
...
function userHasBooked($userId, $date, $startTime, $endTime)
{
    global $dsn, $pdoOptions;
    try {
        $pdo = new PDO($dsn, PARAMS['USER'], PARAMS['PASSWORD'], $pdoOptions);
        $stmt = $pdo->prepare(
            "SELECT COUNT(*) FROM reservations 
             WHERE user_id = ? AND date = ? AND start_time >= ? AND end_time <= ? AND cancelled = 0"
        );
        $stmt->execute([$userId, $date, $startTime, $endTime]);
        return $stmt->fetchColumn() > 0;
    } catch (PDOException $e) {
        die("Error checking user booking: " . $e->getMessage());
    }
}
