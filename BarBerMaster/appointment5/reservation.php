                                                ...
                                                <tbody>
                                                    ...
                                                    for ($j = 0; $j < count($timeslots); $j++) {
                                                        ...
                                                        for ($i = 0; $i < 7; $i++) {
                                                            ...
                                                            $timeslotStart = new DateTime("$date $buttonStart");
                                                            $timeslotEnd = new DateTime("$date $buttonEnd");

                                                            // Check if the date is in the past, if it's Sunday, if the current time is past the end of the timeslot
                                                            if ($date < date('Y-m-d') || $day == 'Sunday' || $currentTime > $timeslotEnd) {
                                                                ...
                                                                $userBooked = userHasBooked($user_id, $date, $buttonStart, $buttonEnd);
                                                                if ($userBooked) {
                                                                    $buttonColor = '#d3d3d3'; // Grey color for already booked slots
                                                                    $disabled = 'disabled';
                        ...
                        <form id="addReservationForm" method="post" action="add_reservation.php" class="archive-form">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="frizer_id" class="form-select" id="frizer_id">
                                    <?php
                                    $frizers = $pdo->query("SELECT frizer_id, frizer_name FROM frizers WHERE salon_id = 5")->fetchAll(PDO::FETCH_ASSOC);
                            ...
                            <div class="mb-3">
                                ...
                                <select name="treatment_id" class="form-select" id="treatment_id">
                                    <?php
                                    $treatments = $pdo->query("SELECT treatment_id, treatment_name, treatment_price FROM treatment WHERE salon_id = 5")->fetchAll(PDO::FETCH_ASSOC);
